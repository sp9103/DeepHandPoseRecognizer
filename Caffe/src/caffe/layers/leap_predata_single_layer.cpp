...
#include "caffe/layers/leap_predata_single_layer.hpp"
...
void LeapPredataSingleLayer<Dtype>::DataLayerSetUp(const vector<Blob<Dtype>*>& bottom,
  ...
  top[0]->Reshape(batch_size_, 1, height_, width_);					//[0] single
  ...
  for (int i = 0; i < ThreadCount; i++){
	  LoadThread[i] = std::thread(&LeapPredataSingleLayer::LoadFuc, this, ThreadCount, i);
  }
}

template <typename Dtype>
void LeapPredataSingleLayer<Dtype>::Reset(Dtype* data, Dtype* labels, int n) {
  CHECK(data);
  CHECK(labels);
...
template <typename Dtype>
void LeapPredataSingleLayer<Dtype>::set_batch_size(int new_size) {
  /*CHECK(!has_new_data_) <<
...
template <typename Dtype>
void LeapPredataSingleLayer<Dtype>::Forward_cpu(const vector<Blob<Dtype>*>& bottom,
	const vector<Blob<Dtype>*>& top) {
...
template <typename Dtype>
void LeapPredataSingleLayer<Dtype>::Leap_PredataLoadAll(const char* datapath){
	WIN32_FIND_DATA ffd;
...
template <typename Dtype>
bool LeapPredataSingleLayer<Dtype>::fileTypeCheck(char *fileName){
	size_t fileLen;
...
template <typename Dtype>
void LeapPredataSingleLayer<Dtype>::LoadFuc(int totalThread, int id){
...
INSTANTIATE_CLASS(LeapPredataSingleLayer);
REGISTER_LAYER_CLASS(LeapPredataSingle);
