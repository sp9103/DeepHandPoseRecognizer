...
#include "caffe/layers/leap_single_data_layer.hpp"
...
void LeapSingleDataLayer<Dtype>::DataLayerSetUp(const vector<Blob<Dtype>*>& bottom,
  ...
  top[0]->Reshape(batch_size_, 1, height_, width_);					//[0] streo
  ...
  LoadThread = std::thread(&LeapSingleDataLayer::LoadFuc, this);
}

template <typename Dtype>
void LeapSingleDataLayer<Dtype>::Reset(Dtype* data, Dtype* labels, int n) {
  CHECK(data);
  CHECK(labels);
...
template <typename Dtype>
void LeapSingleDataLayer<Dtype>::set_batch_size(int new_size) {
  /*CHECK(!has_new_data_) <<
...
template <typename Dtype>
void LeapSingleDataLayer<Dtype>::Forward_cpu(const vector<Blob<Dtype>*>& bottom,
	const vector<Blob<Dtype>*>& top) {
	...
	for (int i = 0; i < batch_size_; i++){
		...
		caffe_copy(height_ * width_ * channels_, streoImg.ptr<Dtype>(0), left_data);
...
template <typename Dtype>
void LeapSingleDataLayer<Dtype>::Leap_LoadAll(const char* datapath){
	WIN32_FIND_DATA ffd;
...
template <typename Dtype>
bool LeapSingleDataLayer<Dtype>::fileTypeCheck(char *fileName){
	size_t fileLen;
...
template <typename Dtype>
void LeapSingleDataLayer<Dtype>::LoadFuc(){
	...
	for (int i = 0; i < ThreadLimit; i++){
		...
		FileLoadThread[i] = std::thread(&LeapSingleDataLayer::ReadFuc, this, srcPath);
	}
	...
	while (1){
		...
		if (label_count < ThreadLimit){
			...
			if (FileLoadThread[ThreadIdx].joinable())
				...
				printf("noting.\n");
			FileLoadThread[ThreadIdx] = std::thread(&LeapSingleDataLayer::ReadFuc, this, srcPath);
...
template <typename Dtype>
void LeapSingleDataLayer<Dtype>::ReadFuc(FilePath src){
	//Left & Right load
	...
	cv::Mat tempStreoMat(height_, width_, CV_32FC1);
	...
	for (int h = 0; h < leftImg.rows; h++){
		for (int w = 0; w < leftImg.cols; w++){
			tempStreoMat.at<float>(width_*h + w) = (float)leftImg.at<uchar>(h, w) / 255.0f;
	...
	//store
	save_mtx.lock();
	left_blob.push_back(tempStreoMat);
	label_blob.push_back(src.id);
	save_mtx.unlock();
}

INSTANTIATE_CLASS(LeapSingleDataLayer);
REGISTER_LAYER_CLASS(LeapSingleData);
